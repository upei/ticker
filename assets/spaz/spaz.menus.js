...
Spaz.Menus.initAll = function() {
	...
	if(air.NativeApplication.supportsDockIcon){ // dock on OS X
		...
		// 	
		// /*
		// 	Don't make this one weak referenced -- it seems to get cleaned up by GC
		// */
...
Spaz.Menus.createRootMenu = function(type){
	...
	var miReposition = new air.NativeMenuItem("Reset position");
	miReposition.addEventListener(air.Event.SELECT,function(event) {
		Spaz.Windows.resetPosition();
	});
	menu.addItem(miReposition);
